...
impl KMeans {
    ...
    pub fn fit(&mut self, X: &Array2<f64>) {
        ...
        self.centroids = Some(X.select(Axis(0), &self.init_centroids(n_samples, &mut rng)).to_owned());
    ...
    fn init_centroids(&self, n_samples: usize, rng: &mut rand::rngs::ThreadRng) -> Vec<usize> {
        let mut indices: Vec<usize> = (0..n_samples).collect();
        indices.shuffle(rng);
        indices.into_iter().take(self.n_clusters).collect()
    }
    ...
    fn update_centroids(&self, X: &Array2<f64>, labels: &Array1<usize>) -> Array2<f64> {
        ...
        for (i, sample) in X.outer_iter().enumerate() {
            let label = labels[i];
            new_centroids.row_mut(label).assign(&sample);
            counts[label] += 1;
        }
...
mod tests {
    ...
    fn test_single_cluster() {
        ...
        let mut model = KMeans::new(1, 100, 1e-4);
        model.fit(&X);
